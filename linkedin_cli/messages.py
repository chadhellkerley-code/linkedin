        ...
        while True:
            ...
            for account in accounts:
                ...
                while sent_for_account < per_account:
                    ...
                    session_file = os.path.join(SESSIONS_DIR, f"account_{account['id']}.json")
                    success, response = playwright_client.send_message_real(
                        session_file, lead["profile_url"], message
                    )
                    if success:
                        self._log_message(account["id"], lead["id"], "ENVIADO")
                        self._update_account_activity(account["id"], True)
                        print(f"[{timestamp}] Cuenta {alias} → Lead {lead_name}: ENVIADO")
                    else:
                        error = response or "Error desconocido"
